...
Solution MDRC(int const& n, int const& d, int const&k, Dataset const& points, vector<vector<int>> const& comb_set1, vector<vector<int>> const& comb_set2, 
    int const& level, vector<array<Weight, 2>> const& R, map<vector<Weight>, KSet> & ksets)
{
    ...
    for(vector<int> const& comb2: comb_set2){
        vector<Weight> imv = transform_to_imv_by_comb2(d, div_dim, comb2, R);
        if(ksets.count(imv) == 0){//对应kset还未构造
            KSet kset = TopK_nth(points, n, k, transform_imv_to_uv(d, imv));
            ksets.insert({imv, kset});
        }
    }
    vector<array<Weight, 2>> LR = R, RR = R;//构造新的探索region
    LR[div_dim][1] = (R[div_dim][0] + R[div_dim][1])/2.0;//划分坐标区间从新构造
    RR[div_dim][0] = (R[div_dim][0] + R[div_dim][1])/2.0;//划分坐标区间从新构造
    //if(k==1) cout << R[0][0] << "," << R[0][1] << "," << level+1 << " ";
...
{
    vector<array<Weight, 2>> R(d-1, {0.0, 1});//使用极坐标系, 有d-1个角, [0.0*pi/2, 1*pi/2]^(d-1), 省去pi/2
    ...
    map<vector<Weight>, KSet> ksets;
    for(vector<int> const& comb1: comb_set1){
        vector<Weight> imv = transform_to_imv_by_comb1(d, comb1, R);
        KSet kset = TopK_nth(points, n, k, transform_imv_to_uv(d, imv));
        ksets.insert(map<vector<Weight>, KSet>::value_type(imv, kset)); //({imv, kset});
    }
