...
inline
ostream & operator << (ostream & stream, vector<Weight> const& vec){
    string str;
    for (Weight val: vec){
        str.append(to_string(val));
        str.append(", ");
	}
    return stream << "<" << str << "size:" << vec.size() << ">";
}
inline
ostream & operator << (ostream & stream, vector<array<Weight, 2>> const& R){
    string str;
    for (array<Weight, 2> pair: R){
        str.append("[");
        str.append(to_string(pair[0]));
        str.append(",");
        str.append(to_string(pair[1]));
        str.append("], ");
	}
    return stream << "(" << str << "size:" << R.size() << ")";
}
